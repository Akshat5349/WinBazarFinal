...
class ChartController extends GetxController {
  ...
  void resultChartAPI() async {
    var body = {'market_slug': market_slug};
    try {
      resultChart.clear();
      var res =
          await ApiProvider().postRequest2(apiUrl: '/panel-chart', data: body);
      ...
      for (var i = 0; i < resultChart.length; i++) {
        ...
        for (var j = 0; j < 7; j++) {
          resultChart[i]['data'][j]['date'] =
              outputFormat.format(startDate.add(Duration(days: j)));
          ;
        }
      }
      print('Result Chart Data: ${resultChart}');
      loading.value = false;
    } catch (e) {
      print(e.toString());
    }
